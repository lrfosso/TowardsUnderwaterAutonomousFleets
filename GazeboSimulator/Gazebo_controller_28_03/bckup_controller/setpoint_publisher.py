    ...
    def timer_callback(self):
        ...
        if(len(self.wp_x) > 1):
            ...
            if((self.wp_t[len(self.wp_t)-1]) <= current_time):  # if the last waypoint has been reached, set reference to last waypoint
                ...
                self.a_x[0], self.a_x[1], self.a_x[2], self.a_x[3] = self.cubic_trajectory_parameter_generation(self.wp_x[self.j],0.2,self.wp_x[self.j+1],0.05,current_time,self.wp_t[self.j+1])
                self.a_y[0], self.a_y[1], self.a_y[2], self.a_y[3] = self.cubic_trajectory_parameter_generation(self.wp_y[self.j],0.2,self.wp_y[self.j+1],0.05,current_time,self.wp_t[self.j+1])
                self.a_z[0], self.a_z[1], self.a_z[2], self.a_z[3] = self.cubic_trajectory_parameter_generation(self.wp_z[self.j],0.2,self.wp_z[self.j+1],0.05,current_time,self.wp_t[self.j+1])
