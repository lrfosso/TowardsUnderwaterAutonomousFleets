...
class Bluerov2PubSubNode(Node):
    ...
    def talker_callback(self):
        ...
        q0ref = self.mpc1.q_0_setp
        e1ref = self.mpc1.e_1_setp
        e2ref = self.mpc1.e_2_setp
        e3ref = self.mpc1.e_3_setp

        x = self.odometry_list[0]
        y = self.odometry_list[1]
        z = self.odometry_list[2]

        xref = self.mpc1.x_setp
        yref = self.mpc1.y_setp
        zref = self.mpc1.z_setp

        cost = ((((q0*q0ref+e1*e1ref+e2*e2ref+e3* e3ref)**2-1)**2 )
            +(-e1ref*q0+q0ref*e1-e3ref*e2+e2ref*e3)**2
            +(-e2ref*q0+e3ref*e1+q0ref*e2-e1ref*e3)**2
            +(-e3ref*q0-e2ref*e1+e1ref*e2+q0ref*e3)**2)
        costlin = 50*(1.8*(x - xref)**2 + 3*(y - yref)**2 +  2*(z - zref)**2)
